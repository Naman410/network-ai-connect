...
const Results = () => {
  const { matches, matchResult } = useMatchContext();
  const [showConfetti, setShowConfetti] = useState(false);
  const navigate = useNavigate();
  const { refreshProfile } = useAuth();

  useEffect(() => {
    // Refresh user profile to get updated LLM count
    refreshProfile();
  }, [refreshProfile]);
  ...
  return (
    <ProtectedRoute>
      <PageWrapper>
        ...
        {matchResult?.llmLimitReached && (
          <div className="text-center mb-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-orange-100 dark:bg-orange-900/20 text-orange-800 dark:text-orange-200 rounded-lg border border-orange-200 dark:border-orange-800">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              You've reached your AI matching limit. Using manual matching algorithm.
            </div>
          </div>
        )}
        ...
        {matches && (
          <div className="container mx-auto max-w-[900px]">
            <div className="relative grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-10 mb-12 px-4 sm:px-6 fade-slide">
              {visibleMatches.map((m) => (
                <div key={m.discord} className="flex justify-center mx-auto">
              ...
              {Array.from({ length: skeletonFill }).map((_, idx) => (
                <div key={`skeleton-${idx}`} className="flex justify-center mx-auto">
                  <div className="w-[350px]">
                    <div className="h-[350px] glass card-radius shadow-card animate-pulse-skel bg-gray-100 dark:bg-slate-800 flex flex-col p-5">
